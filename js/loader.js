document.addEventListener('DOMContentLoaded', async function() {
  ...
  try {
    setStatus('Loading bundle data...');
    const [manifest, template_raw, extResources] = await Promise.all([
      fetch('data/manifest.json').then(r => r.json()),
      fetch('data/template.json').then(r => r.json()),
      fetch('data/ext_resources.json').then(r => r.json()).catch(() => [])
    ]);
